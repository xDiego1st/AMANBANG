<div class="nk-content-body">
    <div class="nk-block-head nk-block-head-sm">
            ...
            <div class="nk-block-head-content">
                ...
                <p class="text-black"><span class="badge bg-outline-info badge-dim">Welcome {{ $user->name }}</span>
                    ...
                    <a href="#" class="collapsed badge bg-outline-success badge-dim" data-bs-toggle="collapse"
                        ...
                        aria-expanded="false">#{{ $this->loadDetailPemohon()?->nomor_registrasi_simbg }}</a>
                <div class="nk-block-des">
                    @if ($this->loadDetailPemohon()?->nomor_registrasi_simbg)
                        <p>Anda Mempunyai total {{ $this->loadDokumen()->count() }} Berkas yang harus melalui proses
                    ...
                    @else
                        <a href="#" class="collapsed" data-bs-toggle="collapse" data-bs-target="#faq-q1"
                            ...
                            <div class="nk-block-des text-soft">
                                <small><span class="badge bg-danger"> Klik Disini untuk Input Nomor
                                        Registrasi SIMBG</span></small> -
                                <small>Silahkan Download KRK Yang telah disetujui dan lakukan pengajuan pada Simbg untuk
                    ...
                    <div class="toggle-expand-content" data-content="pageMenu">
                        <ul class="nk-block-tools g-3">
                            <li class="nk-block-tools-opt">
                                <a href="javascript:void(0)" target="_blank" wire:click.prevent='DownloadKRK'
                            ...
                            <li class="nk-block-tools-opt">
                                <a href="javascript:void(0)" target="_blank" wire:click.prevent='downloadBA'
        ...
        <div class="accordion-item">
            <div class="accordion-body collapse" id="faq-q1" data-bs-parent="#faqs" style="" wire:ignore.self>
                <div class="accordion-inner">
                    <div class="form-group">
                        <div class="gx-3 d-flex">
                            <input type="text" class="form-control" id="nomor_registrasi_simbg"
                                placeholder="Masukkan Nomor Registrasi SIMBG" wire:model.live='nomor_registrasi_simbg'>

                            <a href="#" class="btn btn-icon btn-primary d-md-none"
                                wire:click.prevent='saveNomorRegistrasiSimbg'><em class="icon ni ni-save-fill"></em></a>
    ...
    <hr>
    <div class="nk-block {{ !$this->loadDetailPemohon()?->nomor_registrasi_simbg ? 'lock-overlay' : '' }}">
        @if (!$this->loadDetailPemohon()?->nomor_registrasi_simbg)
            ...
            <div class="col-lg-9">
                <div class="row g-gs">
                    @foreach ($this->loadDokumen() as $i)
                        @php
                            $status_dokumen = $this->loadStatusDokumen($i->id);
                        @endphp
                                        ...
                                        <div class="project-head">
                                            <a href="javascript:void(0)" class="project-title">
                                                <img class="user-avatar sm" style="height: 40px;width: 40px"
                                                    src="https://cdn1.vectorstock.com/i/1000x1000/15/10/agreement-document-line-icon-contract-file-vector-44981510.jpg">
                                                <div class="project-info">
                                                    <h6 class="title">{{ $i->nama_dokumen }}</h6>
                                                    <span
                                                        class="sub-text">#{{ $this->loadDetailPemohon()?->nomor_registrasi_simbg }}</span>
                                                </div>
                                            </a>
                                            <div class="drodown">
                                        ...
                                        <div class="project-progress">
                                            <div class="project-progress-details">
                                                ...
                                                <div class="project-progress-percent">
                                                    {{ config('styles.loading_status_upload.' . $status_dokumen?->status . '.v') }}%
                                                </div>
                                            </div>
                                            <div class="progress progress-pill progress-md bg-light" wire:ignore>
                                                <div class="progress-bar"
                                                    data-progress="{{ config('styles.loading_status_upload.' . $status_dokumen?->status . '.v') }}">
                                                </div>
                                            </div>
                                            <br>
                                            <button class="btn btn-block btn-dim btn-info disabled">Lihat
                                                Informasi</button>
                                        </div>
                                        @if ($status_dokumen)
                                            <div class="project-meta">
                                                <span
                                                    class="badge {{ config('styles.status_upload.' . $status_dokumen?->status . '.class') }}">{{ config('styles.status_upload.' . $status_dokumen?->status . '.text') }}</span>
                                                <span class="badge badge-dim bg-primary"><em
                                                        class="icon ni ni-clock"></em><span>{{ Carbon::parse($status_dokumen?->created_at)->diffForHumans() }}</span></span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    @endforeach
                    <div class="col-lg-12">
                        <livewire:data.table-upload-dokumen-pemohon />
                    </div>
                </div>
            </div>
            <div class="col-lg-3">
