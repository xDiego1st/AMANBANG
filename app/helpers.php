...
if (!function_exists('date_today')) {
    ...
    function date_today()
    {
        $str = Carbon::now()->isoFormat('dddd, LL');
...
if (!function_exists('convert_date')) {
    ...
    {
        $formattedDate =
        Carbon::parse($date)->diffInDays() > 1
        ? Carbon::parse($date)->isoFormat('dddd | D MMMM Y, h:mm a')
...
if (!function_exists('convert_date3')) {
...
if (!function_exists('convert_date4')) {
     ...
     * Friday, July 24, 2020
     */
    function convert_date4($date)
    {
        $formattedDate = Carbon::parse($date)->translatedFormat('l, d F Y | h:m');
        return $formattedDate;
    }
}
if (!function_exists('convertPhoneNumber')) {
    function convertPhoneNumber($phoneNumber)
    {
        ...
        // Memastikan nomor dimulai dengan '62' atau '0'
        if (strpos($phoneNumber, '62') === 0) {
            $phoneNumber = '0' . substr($phoneNumber, 2);
        }

        // Memastikan nomor memiliki panjang yang valid
        $length = strlen($phoneNumber);
        if ($length < 11 || $length > 15) {
            return "Nomor HP tidak valid";
        }

        // Memformat nomor dengan tanda hubung
        // Bagian format akan bervariasi tergantung pada panjang nomor
        $formattedNumber = '';
        if ($length <= 12) {
            $formattedNumber = substr($phoneNumber, 0, 4) . '-' .
            substr($phoneNumber, 4, 4) . '-' .
            substr($phoneNumber, 8);
        } elseif ($length <= 13) {
            $formattedNumber = substr($phoneNumber, 0, 4) . '-' .
            substr($phoneNumber, 4, 4) . '-' .
            substr($phoneNumber, 8, 5);
        } elseif ($length <= 15) {
            $formattedNumber = substr($phoneNumber, 0, 4) . '-' .
            substr($phoneNumber, 4, 4) . '-' .
            substr($phoneNumber, 8, 5) . '-' .
            substr($phoneNumber, 13);
        }
